...
export default function MgmtUsersPage() {
  ...
  const setStatus = async (id: string, status: string) => {
    setActionId(id);
    await fetch('/api/admin/users', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, status }) });
    await load();
    setActionId(null);
  };

  const setCapStep = async (id: string, capStep: number) => {
    setActionId(id);
    await fetch('/api/admin/users', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, capStep }) });
    await load();
    setActionId(null);
  };
  ...
  return (
    <div className="mgmt-page">
      ...
      <div className="mgmt-table-wrap" style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px', overflow: 'auto' }}>
        {loading ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '680px' }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={8} style={{ textAlign: 'center', padding: '48px', color: '#94a3b8', fontSize: '14px' }}>No candidates found.</td></tr>
              ) : filtered.map((c, i) => {
                  ...
                  <tr key={c.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <button onClick={() => setCapStep(c.id, Math.max(1, (c.capStep ?? 1) - 1))} disabled={actionId === c.id || (c.capStep ?? 1) <= 1}
