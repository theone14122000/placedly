...
export default function FreelancerRegister() {
  ...
  const inp = (n: string): React.CSSProperties => ({
    display: 'block', width: '100%', padding: '11px 14px', border: `1.5px solid ${focused === n ? '#f97316' : '#e2e8f0'}`,
    borderRadius: '9999px', fontSize: '14px', fontFamily: "'Poppins',sans-serif", color: '#0b0d20',
    background: focused === n ? '#fff' : '#fff7ed', outline: 'none', boxSizing: 'border-box' as const,
    boxShadow: focused === n ? '0 0 0 3px rgba(249,115,22,0.09)' : 'none',
  });
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff7ed', fontFamily: "'Poppins',sans-serif", padding: '24px' }}>
      <div style={{ maxWidth: '480px', width: '100%', textAlign: 'center', background: '#fff', borderRadius: '20px', padding: 'clamp(28px, 6vw, 48px) clamp(20px, 5vw, 36px)', boxShadow: '0 4px 32px rgba(0,0,0,0.07)' }}>
        ...
        <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: '12px', padding: '16px', marginBottom: '24px' }}>
          ...
          <div style={{ fontSize: 'clamp(22px, 6vw, 28px)', fontWeight: 900, letterSpacing: '3px', color: '#f97316' }}>{done}</div>
        </div>
        <Link href="/login" style={{ display: 'block', padding: '12px 28px', background: '#f97316', color: '#fff', borderRadius: '9999px', fontWeight: 700, fontSize: '14px', textDecoration: 'none' }}>Go to Login</Link>
      </div>
    </div>
  );

  return (
    <div style={{ minHeight: '100vh', background: '#fff7ed', fontFamily: "'Poppins',sans-serif", padding: 'clamp(24px, 5vw, 40px) clamp(16px, 4vw, 24px)' }}>
      <style>{`
        .reg-grid-2 {
          display: grid;
      ...
      <div style={{ maxWidth: '560px', margin: '0 auto' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e2e8f0', padding: 'clamp(20px, 5vw, 36px)', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
          ...
          <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <button type="submit" disabled={loading} style={{ display: 'block', width: '100%', padding: '14px', background: loading ? '#fdba74' : '#f97316', color: '#fff', fontWeight: 700, fontSize: '15px', fontFamily: "'Poppins',sans-serif", border: 'none', borderRadius: '9999px', cursor: loading ? 'not-allowed' : 'pointer', textAlign: 'center', marginTop: '4px' }}>
        ...
        <p style={{ fontSize: '12px', color: '#94a3b8', textAlign: 'center', marginTop: '16px' }}>
          Already registered? <Link href="/login" style={{ color: '#f97316', fontWeight: 600 }}>Sign in →</Link>
        </p>
      </div>
    </div>
  );
