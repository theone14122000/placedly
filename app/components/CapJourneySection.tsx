...
/** Extra scroll runway per card, beyond the card's own height. */
...
export default function CapJourneySection({ cms = {} }: { cms?: Cms }) {
  ...
  const updateScrollProgress = useCallback(() => {
    ...
    setFillProgress(clamp(scrolled / scrollable, 0, 1));
  }, []);

  /* -------------------- MEASURE ON MOUNT + IMG LOAD + RESIZE -------------------- */
  useEffect(() => {
    ...
    const handleImgLoad = (): void => measure();
    imgs.forEach((img) => {
  ...
  return (
    ...
    >
        ...
        <div className="placedly-cap-journey-scroll-layout">
          ...
          <div ref={cardsColRef} className="placedly-cap-journey-cards-col">
            <div ref={trackRef} className="placedly-cap-journey-track">
              {DEFAULT_STEPS.map((step, index) => {
                const slotHeight = slotHeights[index];
                const slotStyle: CSSProperties =
                  slotHeight && slotHeight > 0 ? { height: slotHeight } : {};
      ...
      {/* Floating CTA — appears while CAP section is in view */}
      <AnimatePresence>
        {showFloatingCta && (
          <motion.div
          ...
          >
            <div className="placedly-cap-floating-cta-float">
              <Link
                href="/cap/apply"
                className="placedly-cap-floating-cta-btn"
              >
