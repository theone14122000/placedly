...
const BASE_STICKY_TOP = 100; // Base distance from top
const OVERLAP_OFFSET = 60;   // How much each subsequent card overlaps the previous one
...
export default function CapJourneySection({ cms = {} }: { cms?: Cms }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { threshold: 0.6, rootMargin: '-35% 0px -60% 0px' } // Tighter margin to detect active card precisely
    );
    ...
    >
      ...
      <style jsx global>{`
        .placedly-cap-journey {
          ...
          background: #FFFBF4;
          overflow: hidden; /* Prevent horizontal scroll from overlapping shadows */
        }
        ...
        .placedly-cap-journey-rail {
          position: sticky;
          top: ${BASE_STICKY_TOP}px;
          height: calc(100vh - ${BASE_STICKY_TOP}px - 100px);
          display: flex;
        ...
        /* Cards Track - The Key to Overlap */
        ...
        .placedly-cap-journey-track {
          ...
          /* Massive padding bottom ensures enough scroll space for all cards to stack */
          padding-bottom: calc(100vh + 200px); 
        }
        
        .placedly-cap-journey-card {
          position: sticky;
          /* Dynamic top handled in JSX, but fallback here */
          top: ${BASE_STICKY_TOP}px; 
          
          margin-bottom: -${OVERLAP_OFFSET}px; /* Negative margin pulls next card up */
          border-radius: 28px;
          background: #FFFBF4;
        ...
        .placedly-cap-journey-card.is-active {
          opacity: 1;
          transform: scale(1);
          box-shadow: 0 25px 60px -12px rgba(249, 115, 22, 0.25), 0 0 0 1px rgba(249, 115, 22, 0.1);
          z-index: 50 !important; /* Force active to top */
        }

        /* Inactive State - Slightly pushed back */
        ...
        <div className="placedly-cap-journey-scroll-layout">
          ...
          <div ref={cardsColRef} className="placedly-cap-journey-cards-col">
            <div ref={cardsTrackRef} className="placedly-cap-journey-track">
              {DEFAULT_STEPS.map((step, index) => {
                // Calculate staggered top position
                // Card 0: 100px, Card 1: 160px, Card 2: 220px, etc.
                const stickyTop = BASE_STICKY_TOP + (index * OVERLAP_OFFSET);
                
                // Z-index logic: Later cards should naturally be higher, 
                // but Active card MUST be highest.
                const zIndex = activeStep === index ? 100 : index;
